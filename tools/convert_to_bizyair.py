...
def get_bizyair_display_name(class_type: str) -> str:
    bizyair_cls_prefix = bizyair.core.nodes_base.PREFIX
    bizyair_logo = bizyair.core.nodes_base.LOGO
    return f"{bizyair_logo}{bizyair_cls_prefix} {bizyair.core.NODE_DISPLAY_NAME_MAPPINGS.get(class_type, class_type)}"


def convert_to_bizyair(inputs: dict):
    bizyair.core.NODE_CLASS_MAPPINGS

    for x in inputs.copy():
        class_type = inputs[x]["class_type"]
        bizyair_cls_type = f"{bizyair.core.nodes_base.PREFIX}_{class_type}"
        is_converted = False
        if bizyair_cls_type in bizyair.core.NODE_CLASS_MAPPINGS:
