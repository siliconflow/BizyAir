<template>
  ...
  <Sheet v-model:open="showDrawer">
    <SheetContent side="left" class="w-auto min-w-[480px]">
      ...
      <div class="bizyair-profile-primary">
        <div class="bizyair-profile-primary-box">
          API Key:
          <span class="bizyair-profile-password margin-left-10" id="bizyair-profile-password">
            {{ info.api_key }}
          </span>
          <div class="bizyair-profile-operate">
            <vTooltips tips="Edit">
      ...
      <div class="bizyair-profile-primary">
        Level:
        <span class="margin-left-10">{{ levelText }}</span>
      </div>
      <div class="bizyair-profile-primary-box">
        <div class="bizyair-profile-primary" style="display: flex; align-items: center">
          Share ID:
          <span class="margin-left-10" v-if="!isEditingShareId" id="bizyair-profile-share-id">
            {{ info.share_id }}
          </span>
          <input v-else class="margin-left-10" id="bizyair-profile-share-id-edit" v-model="editedShareId"
            @keyup.enter="saveShareId" />
        </div>
        <div class="bizyair-profile-operate">
          <vTooltips tips="Edit" v-if="canEditShareId">
            <span class="bizyair-icon-operate bizyair-icon-edit"></span>1
          </vTooltips>
          <vTooltips v-else :tips="generateShareIDMessage(info.last_share_id_update_at)">
            <span class="bizyair-icon-operate bizyair-icon-edit bizyair-icon-disabled"></span>2
          </vTooltips>
          <vTooltips v-show="isEditingShareId" :tips="'Save'">
            <span class="bizyair-icon-operate bizyair-icon-save" @click="saveShareId"></span>4
          </vTooltips>
          <vTooltips :tips="'Copy'">
            <span class="bizyair-icon-operate bizyair-icon-copy" @click="copyShareId"></span>5
          </vTooltips>
        </div>
      </div>
    </SheetContent>
...
const copyShareId = () => {
  if (navigator.clipboard) {
    ...
    toast({
      description: 'The browser does not support the Clipboard API.',
      // type: 'error'
    });
  }
};
