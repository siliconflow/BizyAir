...
def get_clipvision_file(preset):
    ...
    if preset.startswith("vit-g"):
        pattern = (
            r"(ViT.bigG.14.*39B.b160k|ipadapter.*sdxl|sdxl.*model\.(bin|safetensors))"
        )
    elif preset.startswith("kolors"):
        pattern = r"(clip.vit.large.patch14.336\.(bin|safetensors))"
    else:
        pattern = (
            r"(ViT.H.14.*s32B.b79K|ipadapter.*sd15|sd1.?5.*model\.(bin|safetensors))"
        )
    clipvision_file = [
        e for e in clipvision_list if re.search(pattern, e, re.IGNORECASE)
    ]

    clipvision_file = (
        folder_paths.get_full_path("clip_vision", clipvision_file[0])
        if clipvision_file
        else None
    )
...
def get_ipadapter_file(preset, is_sdxl):
    ...
    # Community's models
        ...
        if is_sdxl:
            pattern = (
                r"(ip_adapter_plus_general|kolors.ip.adapter.plus)\.(safetensors|bin)$"
            )
        else:
    ...
    ipadapter_file = (
        folder_paths.get_full_path("ipadapter", ipadapter_file[0])
        if ipadapter_file
        else None
    )
...
def merge_hiddenstates(x, tiles):
    ...
    for embeds in x:
        ...
        avg_class_token = class_tokens.mean(dim=0, keepdim=True).unsqueeze(
            0
        )  # Average token, shape: [1, 1, embeds[-1]]

        patch_embeds = embeds[:, 1:, :]  # Shape: [num_tiles, tile_size^2, embeds[-1]]
        reshaped = patch_embeds.reshape(
            grid_size, grid_size, tile_size, tile_size, embeds.shape[-1]
        )

        merged = torch.cat(
            [
                torch.cat([reshaped[i, j] for j in range(grid_size)], dim=1)
                for i in range(grid_size)
            ],
            dim=0,
        )

        merged = merged.unsqueeze(
            0
        )  # Shape: [1, grid_size*tile_size, grid_size*tile_size, embeds[-1]]

        # Pool to original size
        pooled = torch.nn.functional.adaptive_avg_pool2d(
            merged.permute(0, 3, 1, 2), (tile_size, tile_size)
        ).permute(0, 2, 3, 1)
        flattened = pooled.reshape(1, tile_size * tile_size, embeds.shape[-1])

        # Add back the class token
        with_class = torch.cat(
            [avg_class_token, flattened], dim=1
...
def merge_embeddings(x, tiles):  # TODO: this needs so much testing that I don't even
    ...
    for embeds in x:
        ...
        merged = merged.unsqueeze(
            0
        )  # Shape: [1, grid_size*tile_size, grid_size*tile_size]

        # Pool to original size
        pooled = torch.nn.functional.adaptive_avg_pool2d(
            merged, (tile_size, tile_size)
        )  # pool to [1, tile_size, tile_size]
...
def encode_image_masked(
    ...
    if tiles > 1:
        ...
        for i in image_split:
            encoded = encode_image_masked_(
                clip_vision, i, mask, batch_size, clipvision_size=clipvision_size
            )
            if not hasattr(embeds_split, "image_embeds"):
                ...
                embeds_split["penultimate_hidden_states"] = encoded[
                ...
                embeds_split["image_embeds"] = torch.cat(
                    (embeds_split["image_embeds"], encoded["image_embeds"]), dim=0
                )
                embeds_split["penultimate_hidden_states"] = torch.cat(
                    (
                        embeds_split["penultimate_hidden_states"],
                        encoded["penultimate_hidden_states"],
                    ),
                    dim=0,
                )

        # embeds_split['last_hidden_state'] = merge_hiddenstates(embeds_split['last_hidden_state'])
        embeds_split["image_embeds"] = merge_embeddings(
            embeds_split["image_embeds"], tiles
        )
        embeds_split["penultimate_hidden_states"] = merge_hiddenstates(
            embeds_split["penultimate_hidden_states"], tiles
        )

        # embeds['last_hidden_state'] = torch.cat([embeds_split['last_hidden_state'], embeds['last_hidden_state']])
        if (
            embeds["image_embeds"].shape[0] > 1
        ):  # if we have more than one image we need to average the embeddings for consistency
            embeds["image_embeds"] = embeds["image_embeds"] * ratio + embeds_split[
                "image_embeds"
            ] * (1 - ratio)
            embeds["penultimate_hidden_states"] = embeds[
                "penultimate_hidden_states"
            ] * ratio + embeds_split["penultimate_hidden_states"] * (1 - ratio)
            ...
            embeds["image_embeds"] = torch.cat(
                [embeds["image_embeds"] * ratio, embeds_split["image_embeds"]]
            )
        ...
        if not hasattr(outputs, "last_hidden_state"):
            ...
            outputs["last_hidden_state"] = torch.cat(
                (
                    ...
                    out[0].to(model_management.intermediate_device()),
                ),
                dim=0,
            )
            outputs["image_embeds"] = torch.cat(
                (
                    ...
                    out[2].to(model_management.intermediate_device()),
                ),
                dim=0,
            )
            outputs["penultimate_hidden_states"] = torch.cat(
                (
                    outputs["penultimate_hidden_states"],
                    out[1].to(model_management.intermediate_device()),
                ),
                dim=0,
            )
