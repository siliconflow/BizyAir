...
class APIClient:
    ...
    async def get_year_cost(
        ...
        server_url = f"{BIZYAIR_SERVER_ADDRESS}/invoices/year_cost"
        params = {}
        if year:
            params["year"] = year
        if api_key:
    ...
    async def get_month_cost(self, month: str = None, api_key: str = None) -> tuple[dict | None, ErrorNo | None]:
        server_url = f"{BIZYAIR_SERVER_ADDRESS}/invoices/month_cost"
        params = {}
        if month:
            params["month"] = month
        if api_key:
    ...
    async def get_day_cost(self, day: str = None, api_key: str = None) -> tuple[dict | None, ErrorNo | None]:
        server_url = f"{BIZYAIR_SERVER_ADDRESS}/invoices/day_cost"
        params = {}
        if day:
            params["day"] = day
        if api_key:
    ...
    async def get_recent_cost(self, api_key: str = None) -> tuple[dict | None, ErrorNo | None]:
        server_url = f"{BIZYAIR_SERVER_ADDRESS}/invoices/recent_cost"
        params = {}
        if api_key:
            params["api_key"] = api_key
